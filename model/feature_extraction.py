...
class PlantFeatureExtractor(nn.Module):
    def __init__(self, *default_mods: str, embedding_size: int = 512, **param_mods: Dict[str, Union[int, float]]):
        ...
        # Expected 3-dimensional input for 3-dimensional weight [640, 768, 1], but got 2-dimensional input of size [4, 768] instead
        ...
        #self.final_feat_extractor = torch.nn.Sequential(
        #    torch.nn.Conv1d(D_in, H, kernel_size = 1, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'),
        #    torch.nn.ReLU(),
        #    torch.nn.Linear(H, D_out),
        #)
        '''
        ...
        x len is 6 as we have erased   
            del x['label']
            del x['plant']
        so there are 6 modularities per x, in a dictionary structure
    ...
    def to(self, *args, **kwargs):
        ...
        # streams - makes Cuda work in parrallel on 2 GPU's (check)
